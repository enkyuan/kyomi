...
describe("queue routing", () => {
  ...
  test("round-trips clip_created reason with the autogenerated title flag", () => {
    const job = {
  ...
  test("rejects a non-boolean replaceClipTitle flag", () => {
    expect(() =>
      parseJob({
        type: "article.extract",
        payload: JSON.stringify({
          ...
          userId: "user-1",
          requestedAt: "2026-07-08T00:00:00.000Z",
          replaceClipTitle: "yes",
        }),
      }),
    ).toThrow();
  });

  test("rejects an autogenerated title without replaceClipTitle true", () => {
    expect(() =>
      parseJob({
        type: "article.extract",
        payload: JSON.stringify({
