...
// MARK: - Enums (raw values mirror the TS wire string unions)
...
extension Color {
  /// Builds a color from a 32-bit ARGB int (`0xAARRGGBB`).
  init(argb: Int) {
...
struct ToastModel: Identifiable, Equatable {
  ...
  init?(arguments: Any?) {
    ...
    // Image bytes are NOT decoded here — the module decodes them off the main
    // thread and attaches the pixels when ready (see ToastImageDecoder). Over
    // the Expo bridge the payload is a base64 string.
    self.expectsImage = (map["image"] as? String)?.isEmpty == false
