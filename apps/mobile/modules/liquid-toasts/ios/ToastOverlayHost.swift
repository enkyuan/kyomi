...
/// A transparent container view added above the React Native content in the
/// **same** window (so Liquid Glass can sample the live app behind it). It only
/// swallows touches that land on an actual toast frame; everything else falls
/// through to RN.
...
/// its `ToastManager` survives across individual toasts (a JS reload is handled
/// explicitly via `flushAll` on handshake).
@MainActor
final class ToastOverlayHost {
  ...
  /// Keeps the overlay frontmost if the app later adds sibling views — e.g. RN
  /// presenting a modal into the same root view.
  func bringToFront() {
