...
/**
 * A transparent [FrameLayout] added above the React Native content in the same
 * activity window. It only swallows touches that land on an actual toast
 * frame; everything else
 * falls through to the sibling RN root view. The Android analog of
...
internal class OverlayHostView(
    ...
    override fun dispatchTouchEvent(ev: MotionEvent): Boolean {
        ...
        // to the RN root view. Once a gesture starts on a toast, keep dispatching
...
/**
 * Installs and owns the overlay for one activity. Created per activity
 ...
 *
 * `ReactActivity` is a `LifecycleOwner` but NOT a `SavedStateRegistryOwner`,
...
internal class OverlayHost(
    ...
    fun install() {
        ...
        // ancestors — and ReactActivity (an AppCompatActivity, not ComponentActivity)
        // never sets owners on its decor. Install ours there, but never clobber a
        // host app's (a host app with an AndroidX activity).
 ...
 * [ViewModelStoreOwner] for the overlay's ComposeView. ReactActivity supplies
