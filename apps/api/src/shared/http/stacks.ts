...
export const requestObservationPlugin = new Elysia({ name: "cronos.http.observation" })
  .use(requestIdMiddleware)
  .use(loggingMiddleware);

/** Drizzle + `pg` pool on context (`db`, `pool`). */
export const databaseAdapterPlugin = new Elysia({ name: "cronos.adapters.database" }).use(dbPlugin);

/**
 * Default stack for `/api/v1` before route handlers: observation + database
 * (session resolution runs inside the versioned group).
 */
export const apiV1AdapterPlugin = new Elysia({ name: "cronos.api.v1.adapters" })
