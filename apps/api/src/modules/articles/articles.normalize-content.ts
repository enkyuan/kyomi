...
function contentFromLegacy(input: ReaderArticleInput) {
  ...
  if (looksLikeHtml(legacy)) {
    ...
    return {
      ...
      contentText: html ? htmlToText(html) : null,
...
function buildStoredBody(input: ReaderArticleInput) {
  if (input.contentHtml?.trim()) {
    ...
    return {
      ...
      contentHtml: html || null,
      contentMarkdown: null,
      contentText: input.contentText?.trim() || (html ? htmlToText(html) : null),
  ...
  if (input.contentText?.trim()) {
    return {
      ...
      contentSource: input.contentSource ?? "text_fallback",
...
export function buildStoredReaderContent(input: ReaderArticleInput): ArticleReaderContentDto {
  ...
  if (storedBody) {
    return {
      contentStatus: storedBody.contentStatus === "pending" ? "partial" : storedBody.contentStatus,
      contentSource: storedBody.contentSource,
      bodyKind: storedBody.bodyKind,
      title: input.title,
      byline: null,
  ...
  return {
    contentStatus: input.contentStatus ?? "failed",
    contentSource: input.contentSource ?? "link_only",
    bodyKind: "fallback",
    title: input.title,
    byline: null,
...
export function buildReadabilityReaderContent(
  ...
  return {
    ...
    bodyKind: extracted.contentHtml ? "html" : "text",
