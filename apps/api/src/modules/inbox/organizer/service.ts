...
import type {
  InboxOrganizerDto,
  InboxOrganizerFolderDto,
  InboxOrganizerSavedItemDto,
  InboxOrganizerTopViewedFeedDto,
} from "./types";
...
export const normalizeOrganizerLimitForTest = normalizeLimit;

async function listFolderSummaries(
  database: DB,
  userId: string,
): Promise<InboxOrganizerFolderDto[]> {
...
async function listTopViewedFeeds(
  ...
  userId: string,
  limit: number,
): Promise<InboxOrganizerTopViewedFeedDto[]> {
  const rows = await database
    .select({
      feedId: feeds.id,
      url: feeds.url,
...
async function listOldestSavedFeedItems(
  ...
  userId: string,
  limit: number,
): Promise<InboxOrganizerSavedItemDto[]> {
  const rows = await database
    .select({
      id: feedItems.id,
...
export async function getInboxOrganizer(
  database: DB,
  userId: string,
  rawLimit?: string | number,
): Promise<InboxOrganizerDto> {
