...
```bash
bun run --cwd tests test:api:integration -- modules/opml/parse.test.ts modules/opml/fetch-url.test.ts modules/opml/store.test.ts modules/opml/queue-jobs.test.ts modules/opml/jobs.test.ts modules/opml/retry.test.ts modules/opml/routes.test.ts modules/opml/recovery.test.ts app/jobs/opml-import-dispatcher.test.ts modules/feeds/subscription/mutations.test.ts
...
3. **Deploy the scheduler with OPML reconciliation.** `runOpmlImportDispatcherLoop` must be running in the scheduler process (dispatch every 1s, reconciliation every 30s, retention piggybacked every 24h) — confirm via scheduler startup logs (`opml.import.dispatcher.started`).
...
| Dispatcher claims per import and per tick                                                                                           | `runOpmlImportDispatcherTick` stats / logs                                                | Whether fair-dispatch bounds (`OPML_DISPATCH_MAX_IMPORTS`, `_PER_IMPORT`, `_TOTAL`) are being hit under real load. |
